...
const static int V_MEM = 16, H_MEM = 16, REG = 16;
...
void run_ml_c0de(string _mem[][V_MEM])
{
	...
	char step;
	long long val;
	string REGISTER[REG] = { "00", "00", "00", "00",
		                     "00", "00", "00", "00",
		                     "00", "00", "00", "00",
		                     "00", "00", "00", "00" };
	...
	do {
		...
		sout = REGISTER[15];									//Convert hex value
...
void op_code(string _mem[][V_MEM], string &_PC, string &_IR)
{
	...
	if (HORIZONTAL == 15 && VERTICAL + 1 >= 16) {			    //If the X is at the last value in the row
	...
	if (PC_LEFT == 16 && PC_RIGHT + 2 >= 16) {					//Same as the _IR we increment it and if it goes 
